...
This package aspires to generate patterns and events in a reasonably
controllable way.  _A work in progress._

The basic idea is to specify the range of shapes for random events.
Then derive some patterns for those events so that tests see enough
matches.

See [`TestMatcher`](matcher_test.go) as an example use.

There's a command-line [tool](cmd/quaminagen) that tries to make it
easy to perform reasonably large-scale matcher testing.
...
Usage of quaminagen:
  ...
  -ops int
    	ops per goroutine (default 1000)
  -other-events int
    	number of other events (default 1000)
  -other-patterns int
    	number of other patterns (default 1000)
  -print-dims
    	print event/pattern dims
  -print-input
    	print input event/pattern
  -threads int
    	number of goroutines (default 4)
      
$ quaminagen -print-dims -print-input -ops 10
